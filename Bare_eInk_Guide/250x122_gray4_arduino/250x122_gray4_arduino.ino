...
// 4-gray panel info card for the 2.13" 250x122 SSD1680 E-Ink display (#6383,
// ribbon FPC-7528B). Flashes a card with the panel's size, chipset, product #,
...
// colstart=8 corrects the Arduino 8px source-RAM offset on this 2.13" panel
// (CircuitPython uses 0). Needs Adafruit_EPD with the MFGN colstart arg.
...
void setup() {
  ...
  display.print("2.13\" 250x122");
  y += row;
  display.setCursor(pad, y);
  display.print("SSD1680  #6383");
  y += row;
  display.setCursor(pad, y);
  display.print("FPC-7528B");
  y += row;
