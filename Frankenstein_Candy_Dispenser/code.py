...
"""
Frankenstein Candy Dispenser with LED Glasses

Original LED Glasses eye code by Phil B.

Button press triggers audio playback and servo to dispense candy
"""
...
glasses.global_current = 100  # Just middlin' bright, please

gamma = 2.6  # For color adjustment. Leave as-is.

# CLASSES & FUNCTIONS ------------------

# The X drawn over each eye while candy is dispensing
X_POINTS = (
    (0, 0), (4, 0),
    (1, 1), (3, 1),
    (2, 2),
...
class Eye:
    ...
    def __init__(self, left, xoff, xshift=0):
        self.left = left  #     Leftmost column on LED matrix
        self.x_offset = xoff  # Horizontal offset (3X space) to fixate
        self.x_shift = xshift  # Horizontal nudge (whole px) for the X eye

    def draw_x(self, color):
        """Clear this eye's section of the matrix and draw an X across it,
        in place of the usual pupil. 'color' is a packed 24-bit RGB int."""
        for y in range(5):
            for x in range(6):
                glasses.pixel(self.left + x, y, 0)
        for point in X_POINTS:
            glasses.pixel(self.left + point[0] + self.x_shift, point[1], color)
...
# Pre-compute the packed color of the X eyes
x_eye_color_packed = gammify(x_eye_color)
...
upper, lower = -4, 23  # Eyelid positions (3X space), start fully open

# Candy dispensing state. A press runs through two phases back to back:
# the sound plays, then the screw turns. Nothing blocks; the main loop
# just checks where it's up to each pass and keeps animating throughout.
STATE_IDLE = 0  #     Waiting for a press
STATE_SOUND = 1  #    Sound effect playing
STATE_DISPENSE = 2  # Servo turning
state = STATE_IDLE
busy_start_time = 0  #     Time of the button press
dispense_start_time = 0  # Time the servo started turning
...
while True:
    ...
    if event:
        ...
        # print(event)
        if event.pressed and state == STATE_IDLE:
            state = STATE_SOUND
            busy_start_time = now
            play_random_sound()
            blink_state = 0
    ...
    if state == STATE_SOUND:
        if (now - busy_start_time) > 0.05 and not mixer.voice[0].playing:
            state = STATE_DISPENSE  # sound done, dispense candy
            dispense_start_time = now
            cont_servo.throttle = servo_throttle
    elif state == STATE_DISPENSE:
        if (now - dispense_start_time) >= candy_portion:
            state = STATE_IDLE
            cont_servo.throttle = 0
            pixels.fill(idle_pixel_color)
    ...
    try:
        ...
        if state != STATE_IDLE:
            phase = (now - busy_start_time) * pulse_rate
            level = 0.5 - 0.5 * math.cos(phase * 2 * math.pi)
            pixels.fill(tuple(int(c * level) for c in dispense_pixel_color))

            # X eyes instead of pupils
            for eye in eyes:
                eye.draw_x(x_eye_color_packed)
            glasses.left_ring.fill(ring_open_color_packed)
            glasses.right_ring.fill(ring_open_color_packed)
            blink_start_time = move_start_time = now
