...
class SpriteManager:
    ...
    # sprite_manager.py - fix the initialization order
    ...
    def load_sprites(self):
        ...
        try:
            ...
            self.mario_head, self.mario_palette = adafruit_imageload.load("/sprites/lars_note.bmp")
            ...
            self.heart_note, self.heart_palette = adafruit_imageload.load("/sprites/heart_note.bmp")
            # Make the background color transparent
            self.heart_palette.make_transparent(0)

            # Load the Drum note bitmap for channel 3 notes
            self.drum_note, self.drum_palette = adafruit_imageload.load("/sprites/drum_note.bmp")
            # Make the background color transparent
            self.drum_palette.make_transparent(0)

            # Load the new sprites for channels 4, 5, and 6
            # Meatball for channel 4
            self.meatball_note, self.meatball_palette = adafruit_imageload.load("/sprites/meatball.bmp")
            self.meatball_palette.make_transparent(0)

            # Star for channel 5
            self.star_note, self.star_palette = adafruit_imageload.load("/sprites/star.bmp")
    ...
    def create_preview_note(self, current_channel, note_bitmap):
        ...
        if current_channel == 0:  # Channel 1 uses Lars note
            ...
            preview_tg = TileGrid(note_bitmap, pixel_shader=self.preview_palettes[current_channel])
    ...
    def load_button_sprites(self):
        ...
        try:
            ...
            self.play_up, self.play_up_palette = adafruit_imageload.load("/sprites/play_up.bmp")
            self.play_up_palette.make_transparent(0)
            self.play_down, self.play_down_palette = adafruit_imageload.load("/sprites/play_down.bmp")
            self.play_down_palette.make_transparent(0)

            # Load stop button images
            self.stop_up, self.stop_up_palette = adafruit_imageload.load("/sprites/stop_up.bmp")
            self.stop_up_palette.make_transparent(0)
            self.stop_down, self.stop_down_palette = adafruit_imageload.load("/sprites/stop_down.bmp")
            self.stop_down_palette.make_transparent(0)

            # Load loop button images
            self.loop_up, self.loop_up_palette = adafruit_imageload.load("/sprites/loop_up.bmp")
            self.loop_up_palette.make_transparent(0)
            self.loop_down, self.loop_down_palette = adafruit_imageload.load("/sprites/loop_down.bmp")
            self.loop_down_palette.make_transparent(0)

            # Load clear button images
            self.clear_up, self.clear_up_palette = adafruit_imageload.load("/sprites/clear_up.bmp")
            self.clear_up_palette.make_transparent(0)
            self.clear_down, self.clear_down_palette = adafruit_imageload.load("/sprites/clear_down.bmp")
