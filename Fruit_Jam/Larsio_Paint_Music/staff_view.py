...
class StaffView:
    ...
    def _generate_x_positions(self):
        ...
        for measure in range(self.MEASURES_PER_LINE):
            ...
            for eighth_pos in range(8):
                x_pos = measure_start + (eighth_pos * self.EIGHTH_NOTE_SPACING) + self.EIGHTH_NOTE_SPACING // 2
    ...
    def create_staff(self):
        ...
        staff_bg_palette[1] = 0x8AAD8A  # Darker, more saturated sage green
        ...
        staff_bg_grid = TileGrid(staff_bg_bitmap, pixel_shader=staff_bg_palette,
                                x=0, y=self.STAFF_Y_START)
        staff_group.append(staff_bg_grid)

        # Create staff lines
        staff_bitmap = Bitmap(self.SCREEN_WIDTH, self.STAFF_HEIGHT, 4)
        staff_palette = Palette(4)
        staff_palette[0] = 0x8AAD8A  # Transparent (sage green background)
        ...
        for x in range(self.START_MARGIN - 5, self.START_MARGIN - 2):
            for y in range(self.STAFF_HEIGHT):
                if y >= self.LINE_SPACING and y <= 5 * self.LINE_SPACING:
                    staff_bitmap[x, y] = 1
        ...
        staff_grid = TileGrid(staff_bitmap, pixel_shader=staff_palette,
                                x=0, y=self.STAFF_Y_START)
    ...
    def create_grid_lines(self):
        ...
        grid_palette[0] = 0x8AAD8A  # Transparent (sage green)
    ...
    def create_playhead(self):
        ...
        playhead_palette[0] = 0x8AAD8A  # Transparent (sage green background)
        ...
        self.playhead = TileGrid(playhead_bitmap, pixel_shader=playhead_palette,
                               x=0, y=self.STAFF_Y_START)
    ...
    def create_highlight(self):
        ...
        highlight_palette[0] = 0x8AAD8A  # Transparent (sage green background)
