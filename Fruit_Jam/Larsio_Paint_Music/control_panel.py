...
class ControlPanel:
    ...
    def __init__(self, screen_width, screen_height):
        ...
        # Transport area
        self.TRANSPORT_AREA_Y = int(screen_height * 0.1) + int((screen_height - int(screen_height * 0.1) - int(screen_height * 0.2)) * 0.95) + 10
    ...
    def create_channel_buttons(self):
        ...
        channel_select_bitmap = Bitmap(self.CHANNEL_BUTTON_SIZE + 6, self.CHANNEL_BUTTON_SIZE + 6, 2)
        ...
        for x in range(self.CHANNEL_BUTTON_SIZE + 6):
            for y in range(self.CHANNEL_BUTTON_SIZE + 6):
                if (x == 0 or x == self.CHANNEL_BUTTON_SIZE + 5 or
                    y == 0 or y == self.CHANNEL_BUTTON_SIZE + 5):
                    channel_select_bitmap[x, y] = 1  # Yellow outline
                else:
                    channel_select_bitmap[x, y] = 0  # Transparent background

        self.channel_selector = TileGrid(channel_select_bitmap, pixel_shader=channel_select_palette,
                              x=7, y=self.CHANNEL_BUTTON_Y - 3)
    ...
    def create_transport_controls(self, sprite_manager):
        ...
        self.stop_button = TileGrid(sprite_manager.stop_up, pixel_shader=sprite_manager.stop_up_palette,
                              x=10, y=button_y)

        self.play_button = TileGrid(sprite_manager.play_up, pixel_shader=sprite_manager.play_up_palette,
                              x=10 + 64 + button_spacing, y=button_y)

        self.loop_button = TileGrid(sprite_manager.loop_up, pixel_shader=sprite_manager.loop_up_palette,
                              x=10 + 2 * (64 + button_spacing), y=button_y)

        self.clear_button = TileGrid(sprite_manager.clear_up, pixel_shader=sprite_manager.clear_up_palette,
                               x=10 + 3 * (64 + button_spacing), y=button_y)
        ...
        return transport_group, self.play_button, self.stop_button, self.loop_button, self.clear_button
    ...
    def _create_fallback_transport_controls(self):
        ...
        for y in range(self.play_center_y - self.play_size//2, self.play_center_y + self.play_size//2):
            width = (y - (self.play_center_y - self.play_size//2)) // 2
            for x in range(self.play_center_x - self.play_size//4, self.play_center_x - self.play_size//4 + width):
        ...
        self.stop_button = TileGrid(self.stop_button_bitmap, pixel_shader=stop_button_palette,
                              x=10, y=self.SCREEN_HEIGHT - 40)
        self.play_button = TileGrid(self.play_button_bitmap, pixel_shader=play_button_palette,
                              x=10 + self.BUTTON_WIDTH + self.BUTTON_SPACING,
                              y=self.SCREEN_HEIGHT - 40)
        self.loop_button = TileGrid(self.loop_button_bitmap, pixel_shader=loop_button_palette,
                              x=10 + 2 * (self.BUTTON_WIDTH + self.BUTTON_SPACING),
                              y=self.SCREEN_HEIGHT - 40)
        self.clear_button = TileGrid(self.clear_button_bitmap, pixel_shader=clear_button_palette,
                               x=10 + 3 * (self.BUTTON_WIDTH + self.BUTTON_SPACING),
                               y=self.SCREEN_HEIGHT - 40)
