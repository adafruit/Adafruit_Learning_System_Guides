...
class NoteManager:
    ...
    def _create_note_positions(self):
        ...
        note_positions.append(bottom_line_y + self.LINE_SPACING + self.LINE_SPACING // 2)  # 0: B3 (ledger line below staff)
        note_positions.append(bottom_line_y + self.LINE_SPACING)  # 1: Middle C4 (ledger line below staff)
        note_positions.append(bottom_line_y + self.LINE_SPACING // 2)  # 2: D4 (space below staff)
        note_positions.append(bottom_line_y)  # 3: E4 (bottom line)
        note_positions.append(bottom_line_y - self.LINE_SPACING // 2)  # 4: F4 (first space)
        note_positions.append(bottom_line_y - self.LINE_SPACING)  # 5: G4 (second line)
        note_positions.append(bottom_line_y - self.LINE_SPACING - self.LINE_SPACING // 2)  # 6: A4 (second space)
        note_positions.append(bottom_line_y - 2 * self.LINE_SPACING)  # 7: B4 (middle line)
        note_positions.append(bottom_line_y - 2 * self.LINE_SPACING - self.LINE_SPACING // 2)  # 8: C5 (third space)
        note_positions.append(bottom_line_y - 3 * self.LINE_SPACING)  # 9: D5 (fourth line)
        note_positions.append(bottom_line_y - 3 * self.LINE_SPACING - self.LINE_SPACING // 2)  # 10: E5 (fourth space)
        note_positions.append(bottom_line_y - 4 * self.LINE_SPACING)  # 11: F5 (top line)
        note_positions.append(bottom_line_y - 4 * self.LINE_SPACING - self.LINE_SPACING // 2)  # 12: G5 (space above staff)
    ...
    def note_exists_at_position(self, x_pos, y_pos, mario_head, mario_palette):
        ...
        for note_tg in self.notes_group:
            ...
            is_mario = hasattr(note_tg.pixel_shader, "_palette") and len(note_tg.pixel_shader._palette) > 1 and note_tg.pixel_shader._palette[0] == mario_palette[0]
    ...
    def add_note(self, x, y, current_channel, note_palettes, mario_head, mario_palette, heart_note, heart_palette, sound_manager):
        ...
        if current_channel == 0:  # Channel 1 (index 0) uses Mario head
            ...
            note_tg = TileGrid(mario_head, pixel_shader=mario_palette)  # Using mario_head param for drum_note
            # Adjust position offset based on the size
            note_width = mario_head.width
            note_height = mario_head.height
            note_tg.x = x_position - note_width // 2
            note_tg.y = y_position - note_height // 2
        # For channels 4-6, we need to modify the ui_manager.py to pass the proper sprites
        # These will use mario_head and mario_palette parameters since that's what the method expects
        elif current_channel >= 3 and current_channel <= 5:  # Channels 4-6 use custom sprites
            note_tg = TileGrid(mario_head, pixel_shader=mario_palette)  # We'll pass appropriate sprites in ui_manager
            note_width = mario_head.width
            note_height = mario_head.height
            note_tg.x = x_position - note_width // 2
            note_tg.y = y_position - note_height // 2
        ...
        if position_index == 0 or position_index == 1:  # B3 or C4
        ...
        return (True, f"Added: Ch{current_channel+1} {self.note_names[position_index]}")
    ...
    def erase_note(self, x, y, mario_head, mario_palette, sound_manager=None):
        ...
        if note_index is not None:
            ...
            for i, (x_pos, y_pos, midi_note, channel) in enumerate(self.note_data):
                if abs(x_pos - note_x) < 5 and abs(y_pos - note_y) < 5:  # Increased tolerance
                    found_data_index = i
            ...
            if found_data_index is not None and sound_manager is not None:
                x_pos, y_pos, midi_note, channel = self.note_data[found_data_index]

                # If this is a sample-based note (channels 0, 1, or 2), stop the sample
                if channel in [0, 1, 2]:
                    # Try both the direct position and position+channel methods
                    sound_manager.stop_sample_at_position(x_pos, y_pos, channel)

                # Remove the note data
                self.note_data.pop(found_data_index)

                print(f"Erased note at position ({x_pos}, {y_pos}) channel {channel+1}")
            else:
            ...
            if note_index in self.note_to_ledger:
                ...
                # Update remaining note_to_ledger mappings to account for the removed ledger line
                new_note_to_ledger = {}
                for n_idx, l_idx in self.note_to_ledger.items():
                    if n_idx != note_index:  # Skip the note we're removing
