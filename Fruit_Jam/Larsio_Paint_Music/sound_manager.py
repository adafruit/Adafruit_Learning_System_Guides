...
class SoundManager:
    ...
    def __init__(self, audio_output="pwm", seconds_per_eighth=0.25):
        ...
        if self.audio_output_type == "pwm":
            ...
            try:
                ...
                self.audio = audiobusio.I2SOut(
                    bit_clock=board.I2S_BCLK,
                    word_select=board.I2S_WS,
                    data=board.I2S_DIN
                )
        ...
        # - Voices 0-2: WAV samples (3 voices)
        # - Voice 3: Synthio output for channels 3-5
        self.mixer = audiomixer.Mixer(voice_count=6, sample_rate=11025, channel_count=1,
                                    bits_per_sample=16, samples_signed=True)
        self.audio.play(self.mixer)

        # Track which voices are being used for samples
        self.active_voices = [False, False, False, False, False, False]  # First 3 for regular samples, next 3 for playback-only
        ...
        try:
            ...
            try:
                self.drum_samples = {
                    59: audiocore.WaveFile("/samples/kick_01.wav"),  # B3
                    60: audiocore.WaveFile("/samples/kick_01.wav"),  # C4
                    62: audiocore.WaveFile("/samples/kick_01.wav"),  # D4
                    64: audiocore.WaveFile("/samples/snare_01.wav"),  # E4
                    65: audiocore.WaveFile("/samples/snare_01.wav"),  # F4
                    67: audiocore.WaveFile("/samples/snare_01.wav"),  # G4
                    69: audiocore.WaveFile("/samples/chat_01.wav"),  # A4
                    71: audiocore.WaveFile("/samples/chat_01.wav"),  # B4
                    72: audiocore.WaveFile("/samples/chat_01.wav"),  # C5
                    74: audiocore.WaveFile("/samples/ohat_01.wav"),  # D5
                    76: audiocore.WaveFile("/samples/ohat_01.wav"),  # E5
                    77: audiocore.WaveFile("/samples/crash_01.wav"),  # F5
                    79: audiocore.WaveFile("/samples/crash_01.wav"),  # G5
                }
    ...
    def play_notes_at_position(self, notes_data):
        ...
        for x_pos, y_pos, midi_note, channel in notes_data:
            if channel in [0, 1, 2]:  # Sample-based channels
                sample_notes[channel].append((x_pos, y_pos, midi_note))
            elif channel in [3, 4, 5]:  # Synthio channels
                synth_notes[channel].append(midi_note)
            else:  # Other channels (MIDI)
                midi_notes[midi_note] = channel
        ...
        for channel, notes in sample_notes.items():
            for x_pos, y_pos, midi_note in notes:
                ...
                print(f"Playing {channel_names[channel]} sample {closest_note} for note {midi_note} on voice {voice_index-1}")
        ...
        # Log synthio channels that have notes
        for channel, notes in synth_notes.items():
            if notes:
                print(f"Playing synth notes on channel {channel+1}: {notes}")

        if midi_notes:
            print(f"Playing MIDI notes: {list(midi_notes.keys())}")
    ...
    def play_multi_sample(self, midi_note, channel=0):
        ...
        try:
            ...
            channel_names = ["Lars", "Heart", "Drum"]
            print(f"Playing {channel_names[channel]} sample {closest_note} for note {midi_note} on voice {voice_index}")
        except Exception as e:
        ...
        try:
            ...
            if self.preview_mode and self.note_to_release and self.note_to_release_channel == channel:
                try:
                    ...
                    print(f"Released previous preview note on channel {channel+1}")
                except Exception as e:
            ...
            wave_names = ["sine", "triangle", "sawtooth"]
            wave_name = wave_names[channel-3] if 3 <= channel <= 5 else "unknown"
            print(f"Playing synthio note: {midi_note} on channel {channel+1} with {wave_name} wave")
        except Exception as e:
            ...
            try:
                ...
                print(f"Playing fallback note: {midi_note} on channel {channel+1}")
    ...
    def stop_sample_at_position(self, x_pos, y_pos, channel):
        ...
        if position_key in self.playback_voice_mapping:
            ...
            print(f"Stopped sample at position ({x_pos}, {y_pos}) channel {channel+1}: voice {voice_index}")
            return True
        ...
        if simple_key in self.position_to_voice:
            ...
            print(f"Stopped sample at position ({x_pos}, {y_pos}): voice {voice_index}")
            return True
    ...
    def update(self):
        ...
        if self.note_to_release and time.monotonic() >= self.note_release_time:
            try:
                self.synth.release(self.note_to_release)
                print(f"Released preview note on channel {self.note_to_release_channel+1}")
                self.note_to_release = None
                self.note_to_release_channel = None
            except Exception as e:
    ...
    def stop_all_notes(self):
        ...
        try:
            ...
            # As a fallback, try to deinitialize and reinitialize the synth
            try:
                self.synth.deinit()
                self.synth = synthio.Synthesizer(sample_rate=11025)
                self.mixer.voice[5].play(self.synth)
